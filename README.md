...
`tools/compile-papyrus.ps1` canonicalizes only the compiler timestamp, user, and computer fields in each fresh PEX header before hashing it. `tools/normalize-pex-metadata.ps1 -ValidateOnly` verifies that privacy contract without modifying the PEX payload. Bethesda's compiler can assign different internal temporary-symbol names on independent runs, so rebuilt PEX payloads are verified by fresh successful compilation from the same PSC sources rather than by byte identity. Staging the same built inputs remains byte-deterministic.
